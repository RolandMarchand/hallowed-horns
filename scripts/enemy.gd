...
export(state) var _current_state: int = state.IDLE setget _set_current_state
...
onready var body: Area2D = get_node("Path2D/PathFollow2D/Body")
...
func _physics_process(_delta: float) -> void:
	...
	if _player:
		...
		body.global_position).clamped(_vision_length)
...
func _chase() -> void:
	var _current_pos: Vector2 = body.global_position
	var _current_path: PoolVector2Array = _navigation_path
	while _current_path.size():
# warning-ignore:return_value_discarded
		_nav_tween.interpolate_property(body, "global_position", _current_pos, _current_path[0],
...
# warning-ignore:return_value_discarded
		...
		_current_pos = body.global_position
...
func _update_navigation_path() -> void:
	if _player and _navigation:
		_navigation_path = _navigation.get_simple_path(body.global_position,
...
# warning-ignore:return_value_discarded
	_set_current_state(state.CHASE)
...
func _set_current_state(_state: int) -> void:
	if _is_ready:
		_navigation_path = []
# warning-ignore:return_value_discarded
